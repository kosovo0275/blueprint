...
// Variable represents a global Ninja variable
// definition that will be written to the output
// .ninja file. A variable may contain references
// to other global Ninja variables, but circular
...
// Pool represents a Ninja pool that will be written
// to the output .ninja file.
type Pool interface {
	packageContext() *packageContext
	name() string                                        // "foo"
	fullName(pkgNames map[*packageContext]string) string // "pkg.foo" or "path.to.pkg.foo"
...
// Rule represents a Ninja build rule that will be
// written to the output .ninja file.
type Rule interface {
	packageContext() *packageContext
	name() string                                        // "foo"
	fullName(pkgNames map[*packageContext]string) string // "pkg.foo" or "path.to.pkg.foo"
...
// ReparentTo sets the localScope's parent scope to the
// scope of the given package context. This allows a
// ModuleContext and SingletonContext to call a
// function defined in a different Go package and have
// that function retain access to all of the
// package-scoped variables of its own package.
