...
// Patch represents a region of a text buffer to
// be replaced [Start, End) and its Replacement
type Patch struct {
	Start, End  int
	Replacement string
}

// PatchList is a list of sorted, non-overlapping
// Patch objects
type PatchList []Patch

type PatchOverlapError error

// Add adds a Patch to a PatchList. It returns a
// PatchOverlapError if the patch cannot be added.
...
// Apply applies all the Patch objects in PatchList
// to the data from an input ReaderAt to an output
// Writer.
