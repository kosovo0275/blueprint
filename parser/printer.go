...
// printToken a single token, including any
// necessary comments or whitespace between this
// token and the previously printed token
...
// printInLineCommentsBefore prints any in-line
// (single line /* */) comments that appear
// _before_ pos
...
// printEndOfLineCommentsBefore prints any
// comments, including end of line comments, that
// appear _before_ the line specified by pos
...
// requestNewlinesForPos compares the line numbers
// of the previous and current positions to
// determine whether extra newlines should be
// inserted. A second newline is allowed anywhere
...
// _requestNewline asks for a newline to be
// inserted before the next token, but do not
// insert any comments.  Used by the comment
// printers.
...
// requestNewline asks for a newline to be inserted
// before the next token. Also inserts any end-of
// line comments for the current line
...
// requestDoubleNewline asks for two newlines to be
// inserted before the next token. Also inserts any
// end-of line comments for the current line
func (p *printer) requestDoubleNewline() {
	p.requestNewline()
	p.pendingNewline = 2
}

// flushSpace flushes any pending whitespace,
// ignoring pending spaces if there is a pending
// newline
...
// printComment prints a single comment, which may
...
// flush prints any comments that occur after the
// last token, and a trailing newline
...
// pad prints whitespace to pad from column l to
// column max
