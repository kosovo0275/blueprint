...
type Node interface {
	// Pos returns the position of the first token in
	// the Node
	Pos() scanner.Position
	// End returns the position of the character after
	// the last token in the Node
	End() scanner.Position
}

// Definition is an Assignment or a Module at the top
// level of a Blueprints file
...
// Assignment is a variable assignment at the top level
// of a Blueprints file, scoped to the file and and
// subdirs.
...
// Module is a module definition at the top level of a
// Blueprints file
type Module struct {
	Type    string
...
// Property is a name: value pair within a Map, which
// may be a top level Module.
...
// Expression is a Value in a Property or Assignment.
// It can be a literal (String or Bool), a Map, a List,
// an Operator that combines two expressions of the
//bsame type, or a Variable that references and
// Assignment.
type Expression interface {
	Node
	// Copy returns a copy of the Expression that will
	// not affect the original if mutated
	Copy() Expression
	String() string
	// Type returns the underlying Type enum of the
	// Expression if it were to be evalutated
	Type() Type
	// Eval returns an expression that is fully
	// evaluated to a simple type
	// (List, Map, String, or Bool). It will return
	// the same object for every call to Eval().
	Eval() Expression
}

// ExpressionsAreSame tells whether the two values are
// the same Expression. This includes the symbolic
// representation of each Expression but not their
// positions in the original source tree. This does not
// apply any simplification to the expressions before
// comparing them (for example, "!!a" wouldn't be
// deemed equal to "a")
...
// GetProperty looks for a property with the given
// name. It resembles the bracket operator of a
// built-in Golang map.
...
// RemoveProperty removes the property with the given
// name, if it exists.
...
// Text returns the text of the comment with // or /*
// and */ stripped
