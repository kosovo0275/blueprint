...
// Glob returns the list of files and directories that
// match the given pattern but do not match the given
// exclude patterns, along with the list of directories
// and other dependencies that were searched to
// construct the file list. The supported glob and
// exclude patterns are equivalent to filepath.Glob,
// with an extension that recursive glob (** matching
// zero or more complete path entries) is supported.
// Any directories in the matches list will have a '/'
// suffix.
//
// In general ModuleContext.GlobWithDeps or
// SingletonContext.GlobWithDeps should be used
// instead, as they will automatically set up
// dependencies to rerun the primary builder when the
// list of matching files changes.
...
// glob is a recursive helper function to handle
// globbing each level of the pattern individually,
// allowing searched directories to be tracked. Also
// handles the recursive glob pattern, **.
...
// saneSplit is a faster version of
// dir, file := filepath.Dir(path), filepath.File(path)
// with no allocations Similar to filepath.Split, but
// returns "." if dir is empty and trims trailing slash
// if dir is not "/".  Returns ".", "" if path is "."
...
// filterExcludes filters the strings in matches based
// on the glob patterns in excludes. Hierarchical
// (a/*) and recursive (**) glob patterns are supported
...
// Match returns true if name matches pattern using the
// same rules as filepath.Match, but supporting
...
// matchPrefix returns true if the beginning of name
// matches pattern using the same rules as
// filepath.Match, but supporting hierarchical patterns
// (a/*). Recursive globs (**) are not supported,
// they should have been handled in Match().
...
// HasGlob returns true if any string in the list
// contains any glob characters (*, ?, or [).
...
// GlobWithDepFile finds all files and directories that
// match glob. Directories will have a trailing '/'. It
// compares the list of matches against the contents of
// fileListFile, and rewrites fileListFile if it has
// changed. It also writes all of the the directories
// it traversed as dependencies on fileListFile to
// depFile.
//
// The format of glob is either path/*.ext for a single
// directory glob, or path/**/*.ext for a recursive
// glob.
//
...
// WriteFileIfChanged wraps ioutil.WriteFile, but only
// writes the file if the files does not already exist
// with identical contents. This can be used along with
// ninja restat rules to skip rebuilding downstream
// rules if no changes were made by a rule.
