...
// AppendProperties appends the values of properties
// in the property struct src to the property struct
// dst. dst and src must be the same type, and both
// must be pointers to structs.
//
// The filter function can prevent individual
// properties from being appended by returning false,
// or abort AppendProperties with an error by returning
// an error. Passing nil for filter will append all
// properties.
//
// An error returned by AppendProperties that applies
// to a specific property will be an
// *ExtendPropertyError, and can have the property
// name and error extracted from it.
//
// The append operation is defined as appending strings
// and slices of strings normally, OR-ing bool values,
// replacing non-nil pointers to booleans or strings,
// and recursing into embedded structs, pointers to
// structs, and interfaces containing pointers to
// structs. Appending the zero value of a property will
// always be a no-op.
...
// PrependProperties prepends the values of properties
// in the property struct src to the property struct
// dst. dst and src must be the same type, and both
// must be pointers to structs.
//
// The filter function can prevent individual
// properties from being prepended by returning false,
// or abort PrependProperties with an error by
// returning an error. Passing nil for filter will
// prepend all properties.
//
// An error returned by PrependProperties that applies
// to a specific property will be an
// *ExtendPropertyError, and can have the property name
// and error extracted from it.
//
// The prepend operation is defined as prepending
// strings, and slices of strings normally, OR-ing
// bool values, replacing non-nil pointers to booleans
// or strings, and recursing into embedded structs,
// pointers to structs, and interfaces containing
// pointers to structs.  Prepending the zero value of
// a property will always be a no-op.
...
// AppendMatchingProperties appends the values of
// properties in the property struct src to the
// property structs in dst. dst and src do not have to
// be the same type, but every property in src must be
// found in at least one property in dst. dst must be
// a slice of pointers to structs, and src must be a
// pointer to a struct.
//
// The filter function can prevent individual
// properties from being appended by returning false,
...
// properties.
//
// An error returned by AppendMatchingProperties that
// applies to a specific property will be an
// *ExtendPropertyError, and can have the property name
// and error extracted from it.
//
// The append operation is defined as appending
// strings, and slices of strings normally, OR-ing bool
// values, replacing non-nil pointers to booleans or
// strings, and recursing into embedded structs,
// pointers to structs, and interfaces containing
// pointers to structs. Appending the zero value of a
// property will always be a no-op.
...
// PrependMatchingProperties prepends the values of
// properties in the property struct src to the
// property structs in dst.  dst and src do not have to
// be the same type, but every property in src must be
// found in at least one property in dst. dst must be
...
// strings, and recursing into embedded structs,
// pointers to structs, and interfaces containing
// pointers to structs. Prepending the zero value of a
// property will always be a no-op.
...
// ExtendProperties appends or prepends the values of
// properties in the property struct src to the
// property struct dst. dst and src must be the same
// type, and both must be pointers to structs.
//
// The filter function can prevent individual
// properties from being appended or prepended by
// returning false, or abort ExtendProperties with an
// error by returning an error. Passing nil for
// filter will append or prepend all properties.
//
// The order function is called on each non-filtered
// property to determine if it should be appended or
// prepended.
//
// An error returned by ExtendProperties that applies
// to a specific property will be an
// *ExtendPropertyError, and can have the property name
// and error extracted from it.
//
...
// and recursing into embedded structs, pointers to
// structs, and interfaces containing pointers to
// structs. Appending or prepending the zero value of
// a property will always be a no-op.
...
// ExtendMatchingProperties appends or prepends the
// values of properties in the property struct src to
// the property structs in dst. dst and src do not have
// to be the same type, but every property in src must
// be found in at least one property in dst. dst must
// be a slice of pointers to structs, and src must be
// a pointer to a struct.
//
// The filter function can prevent individual
// properties from being appended or prepended by
// returning false, or abort ExtendMatchingProperties
// with an error by returning an error. Passing nil for
// filter will append or prepend all properties.
//
// The order function is called on each non-filtered
// property to determine if it should be appended or
// prepended.
//
// An error returned by ExtendMatchingProperties that
// applies to a specific property will be an
// *ExtendPropertyError, and can have the property name
// and error extracted from it.
//
...
// strings, and recursing into embedded structs,
// pointers to structs, and interfaces containing
// pointers to structs. Appending or prepending the
// zero value of a property will always be a no-op.
