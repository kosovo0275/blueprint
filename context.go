...
// Context contains all the state needed to parse a set
// of Blueprints files and generate a Ninja file. The
// process of generating a Ninja file proceeds through
// a series of four phases. Each phase corresponds with
// some methods on the Context object
//
//         Phase                    Methods
//      ------------    -------------------------------
//   1. Registration    RegisterModuleType,
//                      RegisterSingletonType
//
//   2. Parse           ParseBlueprintsFiles, Parse
//
//   3. Generate        ResolveDependencies,
//                      PrepareBuildActions
//
//   4. Write           WriteBuildFile
//
// The registration phase prepares the context to
// process Blueprints files containing various types of
// modules. The parse phase reads in one or more
// Blueprints files and validates their content against
// the module types that have been registered. The
// generate phase then analyzes the parsed Blueprints
// contents to create an internal representation for
// the build actions that must be performed. This phase
// also performs validation of the module dependencies
// and property values defined in the parsed Blueprints
// files. Finally, the write phase generates the Ninja
// manifest text based on the generated build actions.
type Context struct {
...
// BlueprintError describes a problem that was
// encountered that is related to a particular location
// in a Blueprints file.
type BlueprintError struct {
	Err error            // the error that occurred
	Pos scanner.Position // the relevant Blueprints file location
}

// ModuleError describes a problem that was encountered
// that is related to a particular module in a
// Blueprints file
...
// PropertyError describes a problem that was
// encountered that is related to a particular property
// in a Blueprints file
...
// Variation is a way that a variant of a module
// differs from other variants of the same module. For
// example, two variants of the same module might have
// Variation{"arch","arm"} & Variation{"arch","arm64"}
type Variation struct {
	// Mutator is the axis on which this variation
	// applies, i.e. "arch" or "link"
	Mutator string
	// Variation is the name of the variation on the axis
	// i.e. "arm" or "arm64" for arch, or "shared" or
	// "static" for link.
	Variation string
}

// variationMap stores a map of Mutator to Variation
// to specify a variant of a module.
...
// subset compares this variationMap to another one.
// Returns true if every entry in this map is either
// the same in the other map or doesn't exist in the
// other map.
...
// NewContext creates a new Context object. The created
// context initially has no module or singleton
// factories registered, so the RegisterModuleFactory
// and RegisterSingletonFactory methods must be called
...
// ModuleFactory creates a new Module object. See the
// Context.RegisterModuleType method for details about
// how a registered ModuleFactory is used by a Context.
type ModuleFactory func() (m Module, propertyStructs []interface{})

// RegisterModuleType associates a module type name
// (which can appear in a Blueprints file) with a
// Module factory function. When the given module type
// name is encountered in a Blueprints file during
// parsing, the Module factory is invoked to
// instantiate a new Module object to handle the build
// action generation for the module.If a Mutator splits
// a module into multiple variants, the factory is
// invoked again to create a new Module for each
// variant.
//
// The module type names given here must be unique for
// the context. The factory function should be a named
// function so that its package and name can be
// included in the generated Ninja file for debugging
// purposes.
//
// The factory function returns two values. The first
// is the newly created Module object.The second is a
// slice of pointers to that Module object's properties
// structs. Each properties struct is examined when
// parsing a module definition of this type in a
// Blueprints file. Exported fields of the properties
// structs are automatically set to the property values
// specified in the Blueprints file.The properties
// struct field names determine the name of the
// Blueprints file properties that are used - the
// Blueprints property name matches that of the
// properties struct field name with the first letter
// converted to lower-case.
//
// The fields of the properties struct must be either
// []string, a string, or bool. The Context will panic
// if a Module gets instantiated with a properties
// struct containing a field that is not one these
// supported types.
//
// Any properties that appear in the Blueprints files
// that are not built-in module properties
// (such as "name" and "deps") and do not have a
// corresponding field in the returned module
// properties struct result in an error during the
...
//   }
//
//   func NewMod() (blueprint.Module, []interface{}) {
//       module := new(myModule)
...
//       ctx.RegisterModuleType("my_module", NewMod)
//       // ...
//   }
//
// would support parsing a module defined in a
// Blueprints file as follows:
//
//   my_module {
//       name: "myName",
//       foo:  "my foo string",
...
//   }
//
// The factory function may be called from multiple
// goroutines. Any accesses to global variables must
// be synchronized.
...
// SingletonFactory creates a new Singleton object.
// See the Context.RegisterSingletonType method for
// details about how a registered SingletonFactory is
// used by a Context.
type SingletonFactory func() Singleton

// RegisterSingletonType registers a singleton type
// that will be invoked to generate build actions.
// Each registered singleton type is instantiated and
// invoked exactly once as part of the generate phase.
// Each registered singleton is invoked in registration
// order.
//
// The singleton type names given here must be unique
// for the context. The factory function should be a
// named function so that its package and name can be
// included in the generated Ninja file for debugging
// purposes.
...
// RegisterPreSingletonType registers a presingleton
// type that will be invoked to generate build actions
// before any Blueprint files have been read. Each
// registered presingleton type is instantiated and
// invoked exactly once at the beginning of the parse
// phase. Each registered presingleton is invoked in
// registration order.
//
// The presingleton type names given here must be
// unique for the context. The factory function should
// be a named function so that its package and name can
// be included in the generated Ninja file for
// debugging purposes.
...
// RegisterTopDownMutator registers a mutator that will
// be invoked to propagate dependency info top-down
// between Modules. Each registered mutator is invoked
// in registration order (mixing TopDownMutators and
// BottomUpMutators) once per Module, and the
// invocation on any module will have returned before
// it is in invoked on any of its dependencies.
//
// The mutator type names given here must be unique to
// all top down mutators in the Context.
//
// Returns a MutatorHandle, on which Parallel can be
// called to set the mutator to visit modules in
...
// RegisterBottomUpMutator registers a mutator that
// will be invoked to split Modules into variants. Each
// registered mutator is invoked in registration order
// (mixing TopDownMutators and BottomUpMutators) once
// per Module, will not be invoked on a module until
// the invocations on all of the modules dependencies
// have returned.
//
// The mutator type names given here must be unique to
// all bottom up or early mutators in the Context.
//
// Returns a MutatorHandle, on which Parallel can be
// called to set the mutator to visit modules in
// parallel while maintaining ordering.
...
type MutatorHandle interface {
	// Parallel sets the mutator to visit modules in
	// parallel while maintaining ordering. Calling any
	// method on the mutator context is thread-safe, but
	// the mutator must handle synchronization for any
	// modifications to global state or any modules
	// outside the one it was invoked on.
...
// RegisterEarlyMutator registers a mutator that will
// be invoked to split Modules into multiple variant
// Modules before any dependencies have been created.
// Each registered mutator is invoked in registration
// order once per Module (including each variant from
// previous early mutators). Module order is
// unpredictable.
//
// In order for dependencies to be satisifed in a
// later pass, all dependencies of a module either
// must have an identical variant or must have no
// variations.
//
// The mutator type names given here must be unique to
// all bottom up or early mutators in the Context.
//
// Deprecated, use a BottomUpMutator instead. The only
// difference between EarlyMutator and BottomUpMutator
// is that EarlyMutator runs before the deprecated
// DynamicDependencies.
...
// SetIgnoreUnknownModuleTypes sets the behavior of the
// context in the case where it encounters an unknown
// module type while parsing Blueprints files. By
// default, the context will report unknown module
// types as an error. If this method is called with
// ignoreUnknownModuleTypes set to true then the
// context will silently ignore unknown module types.
//
// This method should generally not be used.It exists
// to facilitate the bootstrapping process.
...
// SetAllowMissingDependencies changes the behavior of
// Blueprint to ignore unresolved dependencies. If the
// module's GenerateBuildActions calls
// ModuleContext.GetMissingDependencies Blueprint will
// not emit any errors for missing dependencies.
...
// fileParseContext tells the status of parsing a
// particular file
type fileParseContext struct {
	// name of file
	fileName string

	// Scope is the scope to use when resolving variables
	Scope *parser.Scope

	// parent is a pointer to the one in the parent
	// directory
	parent *fileParseContext

	// doneVisiting is closed once FileHandler has
...
// ParseFileList parses a set of Blueprints files
// starting with the file at rootFile. When it
// encounters a Blueprints file with a set of subdirs
// listed it recursively parses any Blueprints files
// found in those subdirectories.
//
// If no errors are encountered while parsing the
// files, the list of paths on which the future output
// will depend is returned. This list will include both
// Blueprints file paths as well as directory paths for
// cases where wildcard subdirs are found.
...
// WalkBlueprintsFiles walks a set of Blueprints files
// starting with the given filepaths, calling the given
// file handler on each
//
// When WalkBlueprintsFiles encounters a Blueprints
// file with a set of subdirs listed, it recursively
// parses any Blueprints files found in those
// subdirectories.
//
// If any of the file paths is an ancestor directory of
// any other of file path, the ancestor will be parsed
// and visited first.
//
// the file handler will be called from a goroutine,
...
// cases where wildcard subdirs are found.
//
// visitor will be called asynchronously, and will only
// be called once visitor for each ancestor directory
// has completed.
//
// WalkBlueprintsFiles will not return until all calls
// to visitor have returned.
...
// MockFileSystem causes the Context to replace all
// reads with accesses to the provided map of filenames
// to contents stored as a byte slice.
...
// openAndParse opens and parses a single Blueprints
// file, and returns the results
func (c *Context) openAndParse(filename string, scope *parser.Scope, rootDir string, parent *fileParseContext) (file *parser.File, subBlueprints []fileParseContext, deps []string, errs []error) {
...
// parseOne parses a single Blueprints file from the
// given reader, creating Module objects for each of
// the module definitions encountered. If the
// Blueprints file contains an assignment to the
// "subdirs" variable, then the subdirectories listed
// are searched for Blueprints files returned in the
// subBlueprints return value. If the Blueprints file
// contains an assignment to the "build" variable, then
// the file listed are returned in the subBlueprints
// return value.
//
// rootDir specifies the path to the root directory of
// the source tree, while filename specifies the path
// to the Blueprints file. These paths are used for
// error reporting and for determining the module's
// directory.
func (c *Context) parseOne(rootDir, filename string, reader io.Reader, scope *parser.Scope, parent *fileParseContext) (file *parser.File, subBlueprints []fileParseContext, errs []error) {
...
func (c *Context) findSubdirBlueprints(dir string, subdirs []string, subdirsPos scanner.Position, subBlueprintsName string, optional bool) ([]string, []error) {
...
// cloneLogicModule clones a build logic module by
// calling the factory method for its module type, and
// then cloning property values. Any values stored in
// the module object that are not stored in properties
...
func (c *Context) createVariations(origModule *moduleInfo, mutatorName string, variationNames []string) ([]*moduleInfo, []error) {

	if len(variationNames) == 0 {
		panic(fmt.Errorf("mutator %q passed zero-length variation list for module %q", mutatorName, origModule.Name()))
	}
...
func (c *Context) convertDepsToVariation(module *moduleInfo, mutatorName, variationName string) (errs []error) {
	...
	for i, dep := range module.directDeps {
		if dep.module.logicModule == nil {
			...
			if newDep == nil {
				errs = append(errs, &BlueprintError{
					Err: fmt.Errorf("failed to find variation %q for module %q needed by %q", variationName, dep.module.Name(), module.Name()),
					Pos: module.pos,
				})
				continue
			}
...
func (c *Context) processModuleDef(moduleDef *parser.Module, relBlueprintsFile string) (*moduleInfo, []error) {
...
func (c *Context) addModule(module *moduleInfo) []error {
	...
	module.group = group
	namespace, errs := c.nameInterface.NewModule(newNamespaceContext(module), ModuleGroup{moduleGroup: group}, module.logicModule)
	if len(errs) > 0 {
		for i := range errs {
...
// ResolveDependencies checks that the dependencies
// specified by all of the modules defined in the
// parsed Blueprints files are valid. This means that
// the modules depended upon are defined and that no
...
// blueprintDepsMutator is the default for dependencies
// handling. If the module implements the (deprecated)
// DynamicDependerModule interface then this set
// consists of the union of those module names returned
// by its DynamicDependencies method and those added by
// calling AddDependencies or AddVariationDependencies
// on DynamicDependencyModuleContext.
...
// findMatchingVariant searches the moduleGroup for a
// module with the same variant as module, and returns
// the matching module, or nil if one is not found.
...
func (c *Context) findReverseDependency(module *moduleInfo, destName string) (*moduleInfo, []error) {
	...
	if possibleDeps == nil {
		return nil, []error{&BlueprintError{
			Err: fmt.Errorf("%q has a reverse dependency on undefined module %q", module.Name(), destName),
			Pos: module.pos,
		}}
	}
...
func (c *Context) addInterVariantDependency(origModule *moduleInfo, tag DependencyTag,
	...
	if fromInfo == nil || toInfo == nil {
		panic(fmt.Errorf("AddInterVariantDependency called for module %q on invalid variant", origModule.Name()))
	}
...
// findBlueprintDescendants returns a map linking
// parent Blueprints files to child Blueprints files
// For example, if
// paths = []string{"a/b/c/Android.bp", "a/Blueprints"},
// then
// descendants = {"":[]string{"a/Blueprints"},
// "a/Blueprints":[]string{"a/b/c/Android.bp"}}
...
type visitOrderer interface {
	// waitCount returns the number of modules that this
	// module needs to wait for
	waitCount(module *moduleInfo) int
	// propagate returns the list of modules that are
...
// parallelVisit calls visit on each module,
// guaranteeing that visit is not called on a module
// until visit on all of its dependencies has finished.
...
// updateDependencies recursively walks the module
// dependency graph and updates additional fields based
// on the dependencies. It builds a sorted list of
// modules such that dependencies of a module always
// appear first, and populates reverse dependency links
// and counts of total dependencies. It also reports
// errors when it encounters dependency cycles. This
// should be called after resolveDependencies, as well
//as after any mutator pass has called addDependency
...
// PrepareBuildActions generates an internal
// representation of all the build actions that need
// to be performed. This process involves invoking the
// GenerateBuildActions method on each of the Module
// objects created during the parse phase and then on
// each of the registered Singleton objects.
//
// If the ResolveDependencies method has not already
// been called it is called automatically by this
// method.
//
// The config argument is made available to all of the
// Module and Singleton objects via the Config method
// on the ModuleContext and SingletonContext objects
// passed to GenerateBuildActions.It is also passed to
// the functions specified via PoolFunc, RuleFunc, and
// VariableFunc so that they can compute
// config-specific values.
//
// The returned deps is a list of the ninja files
// dependencies that were added by the modules and
// singletons via the ModuleContext.AddNinjaFileDeps(),
// SingletonContext.AddNinjaFileDeps(), and
// PackageContext.AddNinjaFileDeps() methods.
...
func (c *Context) runMutator(config interface{}, mutator *mutatorInfo, direction mutatorDirection) (deps []string, errs []error) {
...
// cloneModules replaces every build logic module with
// a clone of itself. Prevents introducing problems
// where a mutator sets a non-property member variable
// on a module, which works until a later mutator
...
// spliceModules removes modules[i] from the list and
// inserts newModules... where it was located,
// returning the new slice and the index of the last
// inserted element
...
func (c *Context) generateModuleBuildActions(config interface{}, liveGlobals *liveTracker) ([]string, []error) {
	...
	c.parallelVisit(bottomUpVisitor, func(module *moduleInfo) bool {
		...
		newErrs := c.processLocalBuildActions(&module.actionDefs, &mctx.actionDefs, liveGlobals)
		if len(newErrs) > 0 {
			errsCh <- newErrs
			return true
		}
...
func (c *Context) generateSingletonBuildActions(config interface{}, singletons []*singletonInfo, liveGlobals *liveTracker) ([]string, []error) {
	...
	for _, info := range singletons {
		...
		newErrs := c.processLocalBuildActions(&info.actionDefs, &sctx.actionDefs, liveGlobals)
		errs = append(errs, newErrs...)
		if len(errs) > maxErrors {
			break
		}
...
func (c *Context) processLocalBuildActions(out, in *localBuildActions, liveGlobals *liveTracker) []error {
...
func (c *Context) walkDeps(topModule *moduleInfo, visitDown func(depInfo, *moduleInfo) bool, visitUp func(depInfo, *moduleInfo)) {
	...
	defer func() {
		if r := recover(); r != nil {
			panic(newPanicErrorf(r, "WalkDeps(%s, %s, %s) for dependency %s", topModule, funcName(visitDown), funcName(visitUp), visiting))
		}
	}()
...
func (c *Context) visitAllModules(visit func(Module)) {
	...
	defer func() {
		if r := recover(); r != nil {
			panic(newPanicErrorf(r, "VisitAllModules(%s) for %s", funcName(visit), module))
		}
	}()
...
func (c *Context) visitAllModulesIf(pred func(Module) bool, visit func(Module)) {
	...
	defer func() {
		if r := recover(); r != nil {
			panic(newPanicErrorf(r, "VisitAllModulesIf(%s, %s) for %s", funcName(pred), funcName(visit), module))
		}
	}()
...
func (c *Context) visitAllModuleVariants(module *moduleInfo, visit func(Module)) {

	var variant *moduleInfo

	defer func() {
		if r := recover(); r != nil {
			panic(newPanicErrorf(r, "VisitAllModuleVariants(%s, %s) for %s", module, funcName(visit), variant))
		}
	}()
...
func (c *Context) makeUniquePackageNames(liveGlobals *liveTracker) (map[*packageContext]string, []string) {
...
// AllTargets returns a map all the build target names
// to the rule used to build them. This is the same
// information that is output by running 'ninja -t
// targets all'. If this is called before
// PrepareBuildActions successfully completes then
...
// ModuleTypePropertyStructs returns a mapping from
// module type name to a list of pointers to property
// structs returned by the factory for that module type
...
func (c *Context) ModuleErrorf(logicModule Module, format string, args ...interface{}) error {
...
func (c *Context) VisitDirectDeps(module Module, visit func(Module)) {
	...
	defer func() {
		if r := recover(); r != nil {
			panic(newPanicErrorf(r, "VisitDirectDeps(%s, %s) for dependency %s", topModule, funcName(visit), visiting))
		}
	}()
...
func (c *Context) VisitDirectDepsIf(module Module, pred func(Module) bool, visit func(Module)) {
	...
	defer func() {
		if r := recover(); r != nil {
			panic(newPanicErrorf(r, "VisitDirectDepsIf(%s, %s, %s) for dependency %s", topModule, funcName(pred), funcName(visit), visiting))
		}
	}()
...
func (c *Context) VisitDepsDepthFirst(module Module, visit func(Module)) {
	...
	defer func() {
		if r := recover(); r != nil {
			panic(newPanicErrorf(r, "VisitDepsDepthFirst(%s, %s) for dependency %s", topModule, funcName(visit), visiting))
		}
	}()
...
// WriteBuildFile writes the Ninja manifeset text for
// the generated build actions to w. If this is called
// before PrepareBuildActions successfully completes
