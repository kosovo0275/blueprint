...
type SingletonContext interface {
	...
	// SetNinjaBuildDir sets the value of the
	// top-level "builddir" Ninja variable that
	// controls where Ninja stores its build log
	// files. This value can be set at most one time
	// for a single build, later calls are ignored.
	SetNinjaBuildDir(pctx PackageContext, value string)

	// Eval takes a string with embedded ninja
	// variables, and returns a string with all of
	// the variables recursively expanded. Any
	// variables references are expanded in the
	// scope of the PackageContext.
	...
	// directories that match the specified pattern
	// but do not match any of the patterns in
	// excludes. Any directories will have a '/'
	// suffix. It also adds efficient dependencies
	// to rerun the primary builder whenever a file
	// matching the pattern as added or removed,
	// without rerunning if a file that does not
	// match the pattern is added to a searched
	// directory.
