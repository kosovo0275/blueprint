...
var (
	...
	link = pctx.StaticRule("link",
		blueprint.RuleParams{
			Command:     "GOROOT='$goRoot' $linkCmd -buildmode=pie -extldflags=-pie -o $out $libDirFlags $in",
...
type GoBinaryTool interface {
	...
	// isGoBinary exists so that other packages can't
...
// goPackage is a module for building Go packages.
type goPackage struct {
	...
	// pkgRoot is the root dir in which the package .a
	// file is located. The full .a file path will be
	// "packageRoot/PkgPath.a"
	pkgRoot string

	// archiveFile is the path of the .a file that is to
	// be built.
	archiveFile string

	// testResultFile is the path of the test result file
...
// goBinary is a module for building executable
// binaries from Go sources.
...
// packageRoot returns the module-specific package root
// directory path. This directory is where the final
// package .a files are output and where dependant
...
// testRoot returns the module-specific package root
// directory path used for building tests. The .a files
// generated here will include everything from
