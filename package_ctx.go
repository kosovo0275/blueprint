...
// PackageContext provides a way to create
// package-scoped Ninja pools, rules, and variables. A
// Go package should create a single unexported
// package-scoped PackageContext variable that it uses
// to create all package-scoped Ninja object
// definitions. This PackageContext object should then
// be passed to all calls to define module- or
// singleton-specific Ninja definitions. For example:
//
//     package blah
//
//     import (
...
//
//         myPrivateVar = pctx.StaticVariable(
//            "myPrivateVar",
//            "abcdef",
//         )
//
//         MyExportedVar = pctx.StaticVariable(
//            "MyExportedVar",
//            "$myPrivateVar 123456!",
//         )
//
...
//     // ...
//
//     func (m *MyModule) GenerateBuildActions(
//         ctx blueprint.Module) {
...
// NewPackageContext creates a PackageContext object
// for a given package. The pkgPath argument should
// always be set to the full path used to import the
// package. This function may only be called from a Go
// package's init() function or as part of a
// package-scoped variable initialization.
...
// checkCalledFromInit panics if a Go package's init
// function is not on the call stack.
...
// callerName returns the package path and function
// name of the calling function. The skip argument has
// the same meaning as the skip argument of
// runtime.Callers.
...
// pkgPathToName makes a Ninja-friendly name out of a
// Go package name by replaceing all the '/' characters
// with '.'.  We assume the results are unique, though
// this is not 100% guaranteed for Go package names
// that already contain '.' characters. Disallowing
// package names with '.' isn't reasonable since many
// package names contain the name of the hosting site
// (e.g. "code.google.com"). In practice this probably
// isn't really a problem.
func pkgPathToName(pkgPath string) string {
	return strings.Replace(pkgPath, "/", ".", -1)
}

// Import enables access to the exported Ninja pools,
// rules, and variables that are defined at the package
// scope of another Go package.  Go's visibility rules
// apply to these references - capitalized names
// indicate that something is exported.  It may only
// be called from a Go package's init() function. The
// Go package path passed to Import must have already
// been imported into the Go package using a Go import
// statement. The imported variables may then be
// accessed from Ninja strings as "${pkg.Variable}",
// while the imported rules can simply be accessed as
...
//     }
//
// Note that the local name used to refer to the
// package in Ninja variable names is derived from
// pkgPath by extracting the last path component. This
// differs from Go's import declaration, which derives
// the local name from the package clause in the
// imported package. By convention these names are made
// to match, but this is not required.
...
// ImportAs provides the same functionality as Import,
// but it allows the local name that will be used to
// refer to the package to be specified explicitly. It
// may only be called from a Go package's init()
// function.
...
// StaticVariable returns a Variable whose value does
// not depend on any configuration information. It may
// only be called during a Go package's initialization
// - either from the init() function or as part of a
// package-scoped variable's initialization.
//
// This function is usually used to initialize a
// package-scoped Go variable that represents a Ninja
// variable that will be output. The name argument
// should exactly match the Go variable name, and the
// value string may reference other Ninja variables
// that are visible within the calling Go package.
...
// VariableFunc returns a Variable whose value is
// determined by a function that takes a config object
// as input and returns either the variable value or an
// error. It may only be called during a Go package's
// initialization - either from the init() function or
...
// variable that will be output. The name argument
// should exactly match the Go variable name, and the
// value string returned by f may reference other Ninja
// variables that are visible within the calling Go
// package.
func (p *packageContext) VariableFunc(name string, f func(config interface{}) (string, error)) Variable {
...
// VariableConfigMethod returns a Variable whose value
// is determined by calling a method on the config
// object. The method must take no arguments and return
...
// variable that will be output. The name argument
// should exactly match the Go variable name, and the
// value string returned by method may reference other
// Ninja variables that are visible within the calling
// Go package.
func (p *packageContext) VariableConfigMethod(name string, method interface{}) Variable {
...
func validateVariableMethod(name string, methodValue reflect.Value) {
	...
	if methodType.Kind() != reflect.Func {
		panic(fmt.Errorf("method given for variable %s is not a function", name))
	}
	if n := methodType.NumIn(); n != 1 {
		panic(fmt.Errorf("method for variable %s has %d inputs (should be 1)", name, n))
	}
	if n := methodType.NumOut(); n != 1 {
		panic(fmt.Errorf("method for variable %s has %d outputs (should be 1)", name, n))
	}
	if kind := methodType.Out(0).Kind(); kind != reflect.String {
		panic(fmt.Errorf("method for variable %s does not return a string", name))
	}
}

// argVariable is a Variable that exists only when it
// is set by a build statement to pass a value to the
// rule being invoked. It has no value, so it can never
// be used to create a Ninja assignment statement. It
// is inserted into the rule's scope, which is used for
// name lookups within the rule and when assigning
// argument values as part of a build statement.
...
// StaticPool returns a Pool whose value does not
// depend on any configuration information. It may only
// be called during a Go package's initialization -
// either from the init() function or as part of a
// package-scoped Go variable's initialization.
//
// This function is usually used to initialize a
// package-scoped Go variable that represents a Ninja
// pool that will be output. The name argument should
// exactly match the Go variable name, and the params
// fields may reference other Ninja variables that are
// visible within the calling Go package.
...
// PoolFunc returns a Pool whose value is determined by
// a function that takes a config object as input and
// returns either the pool parameters or an error. It
// may only be called during a Go package's
// initialization - either from the init() function or
// part of a package-scoped variable's initialization.
//
// This function is usually used to initialize a
// package-scoped Go variable that represents a Ninja
// pool that will be output. The name argument should
// exactly match the Go variable name, and the string
// fields of the PoolParams returned by f may
// reference other Ninja variables that are visible
// within the calling Go package.
...
// NewBuiltinPool returns a Pool object that refers to
// a pool name created outside of Blueprint
...
// StaticRule returns a Rule whose value does not
// depend on any configuration information. It may only
// be called during a Go package's initialization -
// either from the init() function or as part of a
// package-scoped Go variable's initialization.
...
// This function is usually used to initialize a
// package-scoped Go variable that represents a Ninja
// rule that will be output. The name argument should
// exactly match the Go variable name, and the params
// fields may reference other Ninja variables that are
// visible within the calling Go package.
//
// The argNames arguments list Ninja variables that may
// be overridden by Ninja build statements that invoke
// the rule. These arguments may be referenced in any
// of the string fields of params.  Arguments can
// shadow package-scoped variables defined within the
// caller's Go package, but they may not shadow those
// defined in another package. Shadowing a
// package-scoped variable results in the
// package-scoped variable's value being used for
// build statements that do not override the argument.
// For argument names that do not shadow package-scoped
// variables the default value is an empty string.
...
// RuleFunc returns a Rule whose value is determined
// by a function that takes a config object as input
// and returns either the rule parameters or an error.
// It may only be called during a Go package's
// initialization - either from the init() function or
// part of a package-scoped variable's initialization.
//
...
// rule that will be output. The name argument should
// exactly match the Go variable name, and the string
// fields of the RuleParams returned by f may reference
// other Ninja variables that are visible within the
// calling Go package.
//
// The argNames arguments list Ninja variables that may
// be overridden by Ninja build statements that invoke
// the rule. These arguments may be referenced in any
// of the string fields of the RuleParams returned by
// f. Arguments can shadow package-scoped variables
// defined within the caller's Go package, but they may
// not shadow those defined in another package.
// Shadowing a package-scoped variable results in the
// package-scoped variable's value being used for build
// statements that do not override the argument. For
// argument names that do not shadow package-scoped
// variables the default value is an empty string.
